...
class TestPositionCollectionOptimization:
    ...
    def sample_with_regions(self):
        ...
        # Token IDs
        input_ids = torch.randint(4, 24, (seq_len,))
        input_ids[0] = 0  # CLS
        input_ids[39] = 2  # EOS
    ...
    def test_all_positions_collected_for_regions_and_germline(self, sample_with_regions):
        ...
        batch_sample = {
            k: v.unsqueeze(0) if isinstance(v, torch.Tensor) else v for k, v in sample.items()
        }
    ...
    def test_positions_overlap_between_regions_and_germline(self, sample_with_regions):
        ...
        hcdr3_positions = set(
            region_masks[AntibodyRegion.HCDR3][0].nonzero(as_tuple=True)[0].tolist()
        )
...
class TestPerPositionEvaluatorIntegration:
    ...
    def mock_model(self):
        ...
        def mock_forward(input_ids=None, attention_mask=None, token_type_ids=None, **kwargs):
        ...
        model.return_value = MaskedLMOutput(logits=torch.randn(1, 40, 32))
