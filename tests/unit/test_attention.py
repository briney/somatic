...
class TestMultiHeadAttention:
    @pytest.fixture
    def attention(self):
        return MultiHeadAttention(d_model=64, n_heads=4, head_dim=16, dropout=0.0, max_seq_len=128)

    def test_forward_shape(self, attention):
        batch, seq_len, d_model = 2, 32, 64
        x = torch.randn(batch, seq_len, d_model)
        token_type_ids = torch.zeros(batch, seq_len).long()  # Ignored but required

        out = attention(x, token_type_ids)
        assert out.shape == x.shape

    def test_with_attention_mask(self, attention):
        batch, seq_len, d_model = 2, 32, 64
        x = torch.randn(batch, seq_len, d_model)
        token_type_ids = torch.zeros(batch, seq_len).long()
        attention_mask = torch.ones(batch, seq_len)
        attention_mask[:, -5:] = 0  # Mask last 5 positions

        out = attention(x, token_type_ids, attention_mask=attention_mask)
        assert out.shape == x.shape
    ...
    def test_deterministic_without_dropout(self, attention):
        ...
        attention.eval()
        out1 = attention(x, token_type_ids)
        out2 = attention(x, token_type_ids)
    ...
    def test_need_weights_returns_attention(self, attention):
        ...
        out, attn_weights = attention(x, token_type_ids, need_weights=True)
    ...
    def test_attention_weights_sum_to_one(self, attention):
        ...
        attention.eval()
        _, attn_weights = attention(x, token_type_ids, need_weights=True)
    ...
    def test_sdpa_and_manual_consistency(self, attention):
        ...
        out_sdpa = attention(x, token_type_ids, need_weights=False)

        # Manual path (need_weights=True)
        out_manual, _ = attention(x, token_type_ids, need_weights=True)
...
class TestChainAwareAttention:
    @pytest.fixture
    def attention(self):
        return ChainAwareAttention(d_model=64, n_heads=4, head_dim=16, dropout=0.0, max_seq_len=128)
...
class TestMultiHeadAttentionQKNorm:
    ...
    def test_qk_norm_none(self):
        ...
        attn = MultiHeadAttention(d_model=64, n_heads=4, head_dim=16, qk_norm="none")
        assert attn.qk_norm_module is None

    def test_qk_norm_layernorm(self):
        """Test forward pass with qk_norm='norm' and LayerNorm."""
        attn = MultiHeadAttention(
            d_model=64, n_heads=4, head_dim=16, qk_norm="norm", norm_type="layernorm"
        )
    ...
    def test_qk_norm_rmsnorm(self):
        ...
        attn = MultiHeadAttention(
            d_model=64, n_heads=4, head_dim=16, qk_norm="norm", norm_type="rmsnorm"
        )
        assert isinstance(attn.qk_norm_module, QKNormModule)
    ...
    def test_qk_norm_learned_scale(self):
        ...
        attn = MultiHeadAttention(d_model=64, n_heads=4, head_dim=16, qk_norm="learned_scale")
    ...
    def test_qk_norm_with_attention_weights(self):
        ...
        attn.eval()
        out, attn_weights = attn(x, token_type_ids, need_weights=True)
...
class TestChainAwareAttentionQKNorm:
    ...
    def test_qk_norm_none(self):
        ...
        attn = ChainAwareAttention(d_model=64, n_heads=4, head_dim=16, qk_norm="none")
    ...
    def test_qk_norm_learned_scale(self):
        """Test forward pass with qk_norm='learned_scale'."""
        attn = ChainAwareAttention(d_model=64, n_heads=4, head_dim=16, qk_norm="learned_scale")
...
class TestSharedQKVChainAwareAttention:
    ...
    def _two_token_type_ids(self, batch: int, seq_len: int) -> torch.Tensor:
        return torch.cat(
            [
    ...
    def test_forward_shape(self, attention):
        batch, seq_len, d_model = 2, 32, 64
        x = torch.randn(batch, seq_len, d_model)
        token_type_ids = self._two_token_type_ids(batch, seq_len)
...
class TestSharedQKVEquivalence:
    ...
    def _copy_weights(self, src: MultiHeadAttention, dst: SharedQKVChainAwareAttention) -> None:
    ...
    def test_same_chain_equivalence(self):
        ...
        out_mha, _ = mha(x, token_type_ids, need_weights=True)
        out_shared, _ = shared(x, token_type_ids, need_weights=True)
...
class TestSharedQKVConfig:
    ...
    def test_config_dict_missing_projection_mode(self):
        # Configs without the projection-mode field default to "separate".
        d = {"vocab_size": 32, "hidden_size": 64, "num_hidden_layers": 2, "num_attention_heads": 2}
    ...
    def test_save_load_roundtrip_shared(self, tmp_path):
        ...
        model = SomaticModel(config)
        save_path = tmp_path / "shared_model"
        ...
        token_type_ids = torch.zeros(2, 8, dtype=torch.long)
        with torch.no_grad():
            out_a = model(input_ids, token_type_ids=token_type_ids).last_hidden_state
            out_b = loaded(input_ids, token_type_ids=token_type_ids).last_hidden_state
...
class TestAttentionTrainingSmoke:
    ...
    def test_forward_backward_step(self, config_kwargs):
        ...
        input_ids = torch.randint(0, config.vocab_size, (2, 8))
        token_type_ids = torch.cat([torch.zeros(2, 4), torch.ones(2, 4)], dim=1).long()
        labels = torch.randint(0, config.vocab_size, (2, 8))

        model.train()
        for _ in range(2):
            optimizer.zero_grad()
            loss = model(input_ids, token_type_ids=token_type_ids, labels=labels).loss
